 ...
 * calls → repeat until finish_task → verifier → deployment status → persist → done.
...
export async function runTurn({ project, session, userText, effort, sse }) {
  ...
  sse.send('done', { summary: finishArgs.summary, verifier_notes: verifierNotes, deployment_status: deploymentStatus });
}
